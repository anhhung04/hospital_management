...
from repository.schemas.patient import Patient, MedicalRecord, PatientProgress, ProgressType, Conclusion, TestResult, Prescription, employee_handle_patient
...
class PatientTest(TestCase):
    def test_insert_patient(self):
        ...
        pp = PatientProgress(treatment_schedule="Test schedule", duration=30, treatment_type="Test type", patient_condition="Test condition", patient_id=p.user_id, status=ProgressType.SCHEDULING, medical_record_id=mr.id)
        db.add(pp)
        db.commit()
        pp_n_db = db.query(PatientProgress).filter(PatientProgress.id == pp.id).first()
        self.assertTrue(pp_n_db is not None)
        self.assertTrue(pp_n_db.treatment_schedule == pp.treatment_schedule)
        self.assertTrue(pp_n_db.duration == pp.duration)
        self.assertTrue(pp_n_db.treatment_type == pp.treatment_type)
        ...
        db.execute(employee_handle_patient.insert().values(employee_id=user_id, progress_id=pp.id, action = "action"))
        db.commit()
        result = db.execute(employee_handle_patient.select().where(employee_handle_patient.c.employee_id == user_id)).fetchall()
        self.assertTrue(result is not None)
        self.assertTrue(len(result) == 1)
        self.assertTrue(result[0][0] == user_id)
        self.assertTrue(result[0][1] == pp.id)
        self.assertTrue(result[0][2] == "action")
